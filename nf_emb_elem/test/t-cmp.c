...
void check_cmp(nf_emb_elem_t a, nf_emb_elem_t b, nf_emb_t nf, const char * s, int ans)
{
	int test;
	...
	if((test = nf_emb_elem_cmp(a, a, nf)))
	{
		...
		printf("b = "); nf_emb_elem_print_pretty(b, nf, s, 10); printf("\n");
...
void test_field1()
{
	...
	fmpq_init(k);
	fmpq_set_si(k, 5, 1);
	nf_emb_init_nth_root_fmpq(nf, k, 2, 20 + n_randint(state, 100));
...
void test_field2()
{
	...
	nf_emb_real_init(nf, pol, emb, 20 + n_randint(state, 100));
